...
from dctap.csvreader import _normalize_header_name
...
def test_normalize_element_name():
    ...
    assert _normalize_header_name("SHAPE ID", element_aliases_dict) == "shapeID"
    assert _normalize_header_name("SHAPE___ID", element_aliases_dict) == "shapeID"
    assert _normalize_header_name("rid", element_aliases_dict) == "rid"
...
def test_normalize_element_name_customized():
    ...
    assert _normalize_header_name("propertyid", element_aliases_dict) == "propertyID"
    assert (
        _normalize_header_name("eigenschaftsidentifikator", element_aliases_dict)
        == "propertyID"
    )
...
def test_normalize_element_name_if_element_aliases_dict_pass_through_unchanged():
    ...
    """
    assert _normalize_header_name("propertyid") == "propertyid"
    assert (
        _normalize_header_name("eigenschaftsidentifikator")
