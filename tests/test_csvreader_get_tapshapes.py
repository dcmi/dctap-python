...
def test_get_tapshapes_twoshapes_first_is_default_because_shapeID_empty():
    ...
    assert expected_shapes["shapes"][1]["shapeID"] == ":author"
    assert len(expected_shapes["shapes"][0]["statement_constraints"]) == 2
    assert type(_get_tapshapes(rows)[0]) == dict
    assert type(_get_tapshapes(rows)[0]["shapes"]) == list
    assert type(_get_tapshapes(rows)[0]["shapes"][0]) == dict
    assert type(_get_tapshapes(rows)[0]["shapes"][0]["statement_constraints"]) == list
    assert type(_get_tapshapes(rows)[0]["shapes"][0]["statement_constraints"][0]) == dict
